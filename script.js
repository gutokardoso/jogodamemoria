...
function reset(){score=0;pairs=0;time=TIMER_TOTAL;lastMatchTime=TIMER_TOTAL;flipped=[];lock=false;canPlay=false;timerEl.textContent=TIMER_TOTAL;timerEl.style.color='';if(scoreEl) if(scoreEl) scoreEl.textContent='0000';pairsEl.textContent='0/8';clearInterval(timerId);updateTimerRing();buildBoard()}
...
function check(){lock=true;const [a,b]=flipped;if(a.dataset.icon===b.dataset.icon){a.classList.add('correct');b.classList.add('correct');const points=getMatchPoints();score+=points;pairs++;if(scoreEl) if(scoreEl) scoreEl.textContent=fmt(score);pairsEl.textContent=pairs+'/8';setTimeout(()=>{a.classList.add('matched');b.classList.add('matched');flipped=[];lock=false;if(pairs===8){const bonus=getFinishBonus();score+=bonus;if(scoreEl) if(scoreEl) if(scoreEl) scoreEl.textContent=fmt(score);end(true)}},650)}else{a.classList.add('wrong');b.classList.add('wrong');setTimeout(()=>{a.classList.remove('flipped','wrong');b.classList.remove('flipped','wrong');flipped=[];lock=false},850)}}
